...
public class AlunoControllerTest {
    ...
    public void testInserirAluno() throws Exception {
        ...
        Aluno aluno = alunos.stream().filter(a -> "Carlos Daniel".equals(a.getNome())).findFirst().orElse(null);
        ...
        mockMvc.perform(get("/alunos-adicionados"))
                ...
                .andExpect(model().attribute("alunosList", hasItem(
                        hasProperty("nome", is("Carlos Daniel"))
                )));
    }
    ...
    public void testListagemAlunos() throws Exception {
        ...
        mockMvc.perform(get("/alunos-adicionados"))
                ...
                .andExpect(model().attribute("alunosList", hasItem(
                        hasProperty("nome", is("Paulo"))
                )))
                .andExpect(model().attribute("alunosList", hasItem(
                        hasProperty("nome", is("Isabele"))
                )));
    }
    ...
    public void testEditarAluno() throws Exception {
        ...
        List<Aluno> alunos = serviceAluno.findAll();
        Aluno savedAluno = alunos.stream().filter(a -> "Carlos Daniel".equals(a.getNome())).findFirst().orElse(null);

        assert savedAluno != null;
        savedAluno.setNome("Carlos Emannuel");
        ...
        mockMvc.perform(get("/editar/" + savedAluno.getId()))
                ...
                .andExpect(model().attribute("aluno", hasProperty("nome", is("Carlos Emannuel"))));
    }
